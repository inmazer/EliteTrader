...
namespace Capture.Interface
{
    ...
    public class CaptureInterface : MarshalByRefObject
    {
        ...
        {
            return GetScreenshot(new TimeSpan(0, 0, 2));
        }
        ...
        public Screenshot GetScreenshot(TimeSpan timeout)
        {
            lock (_lock)
            {
                ...
                SafeInvokeScreenshotRequested(new ScreenshotRequest(_requestId.Value));
        ...
        public IAsyncResult BeginGetScreenshot(TimeSpan timeout, AsyncCallback callback = null)
        {
            Func<TimeSpan, Screenshot> getScreenshot = GetScreenshot;
            
            return getScreenshot.BeginInvoke(timeout, callback, getScreenshot);
        }

        public Screenshot EndGetScreenshot(IAsyncResult result)
        {
            Func<TimeSpan, Screenshot> getScreenshot = result.AsyncState as Func<TimeSpan, Screenshot>;
        ...
        private void SafeInvokeScreenshotRequested(ScreenshotRequest eventArgs)
        {
            ...
            {
                Message(MessageType.Debug, "ScreenshotRequested was null, no listeners");
                return; //No Listeners
            }
            ...
            //Message(MessageType.Debug, "Got ({0}) delegates", dels.Length);
            ...
            foreach (Delegate del in dels)
            {
                ...
                catch (Exception e)
                {
                    //Could not reach the destination, so remove it
                    //from the list
                    Message(MessageType.Error, "Got error invoking the delegate: " + e.ToString());
